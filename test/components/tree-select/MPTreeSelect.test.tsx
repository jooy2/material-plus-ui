...
async function open(screen: { getByRole: (role: string, options?: object) => { click: () => Promise<void> } }) {
...
describe('MPTreeSelect', () => {
  it('opens a tree behind a field', async () => {
    ...
    await render(<MPTreeSelect label="Region" items={ITEMS} defaultExpanded={['europe']} />);
  ...
  it('will not choose a branch unless it is allowed to', async () => {
    ...
    await open(screen);
    await screen.getByText('Europe').click();

    expect(onValueChange).toHaveBeenCalledWith([]);
    // Still open, and the branch opened rather than being chosen.
  ...
  it('keeps only the last choice unless it may hold more than one', async () => {
    ...
    await open(screen);
    await screen.getByText('France').click();
    await screen.getByText('Spain').click();
  ...
  it('refuses a disabled node', async () => {
    ...
    await open(screen);
    await screen.getByText('Japan').click();
  ...
  it('opens every branch a search kept', async () => {
    ...
    await userFillSearch(screen, 'seoul');
  ...
  it('keeps everything under a branch that matched', async () => {
    ...
    await open(screen);
    await userFillSearch(screen, 'europe');
  ...
  it('says so when a search matched nothing', async () => {
    ...
    await open(screen);
    await userFillSearch(screen, 'atlantis');
  ...
  it('offers no search field unless it is asked for', async () => {
    await render(<MPTreeSelect label="Region" items={ITEMS} />);
  ...
  it('submits one hidden input per value', async () => {
    const screen = await render(
      <MPTreeSelect label="Region" items={ITEMS} name="region" multiple defaultValue={['france', 'spain']} />
    );
...
async function userFillSearch(
  screen: { getByRole: (role: string) => { element: () => HTMLElement | SVGElement } },
  text: string
) {
