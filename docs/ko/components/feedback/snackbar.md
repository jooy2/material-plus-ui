...
이 값은 판이 어느 방향으로 움직일지도 정합니다. 스낵바는 스택이 붙어 있는 가장자리에서 200ms 동안 들어오고, 같은 방향으로 나갑니다. 어느 쪽으로 밀어 없앨 수 있는지를 정하는 것도 같은 사실이고, 둘은 일치해야 합니다. 위에서 내려왔는데 위로만 밀어낼 수 있는 판은 자기를 되돌려 달라고 하는 셈입니다.

움직임을 덜 원한다고 밝힌 독자에게는 출발할 곳이 없는 페이드가 남습니다.

스낵바 하나만 provider가 고른 스택을 벗어날 수 있고, `add`에서 같은 이름의 값을 주면 됩니다.

```tsx
snackbar.add({ message: '저장하지 못했습니다', position: 'top-center' });
```

아래쪽 구석이 툴바인 페이지의 오류처럼, _무엇인지 때문에_ 자리가 달라져야 하는 메시지를 위한 것입니다. 취향을 위한 것이 아닙니다. 스낵바가 전부 같은 잘못된 자리에 뜨는 애플리케이션이라면 provider를 옮기세요.

여섯 스택은 안에 무엇이 들었든 첫 렌더부터 모두 페이지에 있고, 이건 숨겨둘 만한 구현 세부가 아닙니다. 각 스택이 자기 `aria-live` 영역인데, 안의 메시지와 같은 순간에 문서에 들어온 영역은 스크린 리더가 비교할 것이 없어 읽지 않습니다. 빈 스택은 자식이 없는 `pointer-events-none` 플렉스 컬럼이라, 아무도 쓰지 않는 다섯 개는 `<div>` 하나씩입니다.
