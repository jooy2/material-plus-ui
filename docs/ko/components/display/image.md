...
버튼은 `preview`가 없는 상자처럼 컨테이너 너비를 채웁니다. 그래서 `preview`를 켜도 그림 크기가 바뀌지 않습니다. `width`만 주면 상자와 함께 버튼도 좁아집니다.
...
- **`<img>`는 블록입니다.** 그래서 자리를 잡아 두지 않은 상자도 그림이 끝나는 곳에서 끝납니다. 인라인 이미지라면 글줄이 글자 아랫부분을 위해 남기는 몇 픽셀이 그림 아래에 생깁니다.
