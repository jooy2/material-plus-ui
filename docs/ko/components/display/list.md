...
## `density`가 그 약속을 지킵니다

밀도라는 축이 생긴 이유가 리스트입니다. 같은 행이 반복되는 컴포넌트라서, 한 번에 몇 개가 보이느냐가 쓸모의 대부분을 결정합니다. `density`는 행이 무엇으로 조판됐는지는 건드리지 않고 여백만 덜어냅니다.

| `density` | `md`의 행 | 같은 높이   |
| --------- | --------- | ----------- |
| `0`       | 56px      | `md` 컨트롤 |
| `-1`      | 52px      | `lg`의 `-2` |
| `-2`      | 48px      | —           |
...
각 단계는 컨트롤 사다리가 이미 이름을 가진 높이에 내려앉습니다. 어느 밀도에서든 행과 옆의 버튼이 맞는 것이 이 때문입니다. 글자는 그대로입니다. 조밀한 리스트는 작아진 글자가 아니라 좁아진 자리입니다.

`xs`는 두 단계에서 멈춰 24px에 머뭅니다. 그 아래로는 행이 손가락으로 누를 수 있는 것이 아니게 됩니다. [`density`](../../design/prop-conventions#density)를 보세요.
