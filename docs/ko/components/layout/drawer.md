...
`modal` 패널은 자기가 붙어 있는 가장자리에서 들어옵니다. 들어올 때는 `emphasized-decelerate`로 400ms, 나갈 때는 `emphasized-accelerate`로 200ms — MD3가 정한 짝 그대로입니다. 이 라이브러리에서 움직이는 떠 있는 면은 이것 하나뿐이고, 나머지 — [MPDialog](../feedback/dialog), [MPMenu](../inputs/menu), [MPPopover](../feedback/popover), [MPSelect](../inputs/select)의 목록 — 는 여전히 페이드만 합니다.

그것들이 따르는 규칙은 글이 담긴 상자는 움직이면 안 된다는 것입니다. 화면 위로 끌려다니는 문장은 독자가 이미 읽으려던 문장이기 때문입니다. 그 규칙은 쓰인 자리에서는 그대로 유효합니다. 메뉴는 포인터가 놓인 행 옆에서 열리고, 다이얼로그는 독자가 읽던 문단 위, 페이지 한가운데에 내려앉습니다.

드로어는 둘 중 어느 쪽도 아닙니다. 드로어는 가장자리에 — 창에서 아무도 읽고 있지 않은 유일한 부분에 — 도착하고, 누군가 버튼을 눌러 기다리고 있기 때문에 도착합니다. 무엇도 무엇의 위로 끌려다니지 않습니다.

대신 페이드가 치르는 비용은, 도착하는 동안 패널에게 남아 있던 유일한 말입니다. 페이드에는 방향이 없어서, 왼쪽 드로어와 오른쪽 드로어는 둘 다 끝날 때까지 같은 사건입니다. 어느 가장자리인지는 모양이 _끝난 뒤에_ 말해 줍니다 — 창 쪽은 각지고, 자유로운 쪽은 잘려 있게. 움직임은 그것을 _도착하는 동안_ 말합니다.

움직임을 덜 원한다고 밝힌 독자에게는 출발할 곳이 없는 페이드가 남습니다. 이 패널이 예전에 하던 것이 정확히 그것입니다.

`standard` 패널은 들어올 때도 나갈 때도 아무 애니메이션이 없습니다. 위가 아니라 레이아웃 안에 있으므로 "닫힘"은 "레이아웃에 없음"입니다. 하나가 열릴 때 움직이는 것은 그 주위의 페이지이고, 페이지를 움직이는 일은 애플리케이션의 몫입니다.
