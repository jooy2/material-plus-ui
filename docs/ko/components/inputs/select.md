...
팝업은 페이드로 나타납니다. 라이브러리의 다른 모든 팝업 — 메뉴, 팝오버, 콤보박스의 목록, 다이얼로그 — 과 같은 200ms, 같은 커브입니다. 불투명도만 바뀌고 그 외에는 아무것도 움직이지 않습니다. 미끄러지거나 확대되는 목록은 포인터가 이미 향하고 있던 행을 옮겨 버리기 때문입니다.
