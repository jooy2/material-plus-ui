...
**고르지 않음은 `null`입니다.** `0`도, `''`도, `-1`도 아닙니다. `items`에 짝이 없는 값은 보여 줄 라벨이 없으므로 트리거가 값 자체를 그립니다 — "아직 안 고름"을 `0`으로 들고 있는 폼은 독자가 채워야 할 그 필드에 `0`이라고 적어 놓게 됩니다.

값이 실재하고 그 행이 아직 도착하지 않은 경우에는 이쪽이 옳은 답입니다. 들고 있고 제출까지 할 값에 대해 아무 말도 하지 않는 필드가 더 나쁩니다. 다만 센티널에 대해서는 옳은 답이 아니며, MUI의 `Select`는 같은 상황에서 빈 칸을 그리므로 옮겨 오는 길에 걸려들기 쉽습니다.
...
커서는 배경이 아니라 **상태 레이어**입니다. [MPMenu](./menu)가 강조된 행 아래에 까는 것과 같은 워시입니다. 배경으로 그리면 선택된 행의 채움을 대체할 수밖에 없고, 그러면 목록이 가장 자주 열리는 바로 그 행에서 커서가 보이지 않게 됩니다. 레이어는 그 위에 겹쳐지고, 나타나는 대신 서서히 짙어집니다.
