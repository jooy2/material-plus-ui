...
목록에 없는 `value`가 어떻게 보이는지도 같은 규칙이 정합니다. 값 자체가 그려지는데, 여기서 그것은 조회 실패가 아니라 누군가 입력한 값이기 때문입니다. **그래서 고르지 않음은 `null`입니다** — `0`도 `''`도 아닙니다. 센티널은 커스텀 값과 구별되지 않고, 필드는 그것을 그대로 보여 줍니다.
