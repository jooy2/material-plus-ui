...
## extended는 지켜볼 수 있는 변화입니다

`extended`를 켜고 끄면 원반이 스타디움으로 **바뀝니다**. 다른 것으로 교체되는 것이 아니라요. MD3가 그리는 것이 그것이고, 스크롤에 맞춰 버튼을 펼치는 호출자가 원하는 것도 그것입니다.

너비를 그냥 전환할 수는 없습니다. 확정된 너비에서 `auto`로 보간하는 방법은 없고, 누가 넘길지 모르는 가장 긴 라벨까지 담을 만큼 큰 `max-width`를 쓰면 그보다 짧은 라벨은 전부 시간의 앞쪽 일부에서 도착을 끝내고 나머지 시간 동안 가만히 있게 됩니다. 그래서 거리를 나눕니다. 버튼은 자기 최소 너비와 안쪽 여백을 올리고, 라벨의 트랙은 `0fr`과 `1fr` 사이를 오가는 그리드 열 하나입니다. 한쪽 끝에서는 라벨에 꼭 필요한 너비로, 다른 쪽 끝에서는 0으로 풀립니다. 둘 다 같은 200ms를 쓰므로 함께 도착합니다.

그래서 라벨은 두 너비 모두에서 DOM에 있습니다. 거기 없는 요소에는 출발할 너비가 없기 때문입니다. 버튼이 원반인 동안 라벨은 0으로 잘려 있고, 두 번 읽히지도 않습니다. `aria-label`이 버튼의 이름이고, 이름은 내용에 더해지는 것이 아니라 내용을 대체합니다.
