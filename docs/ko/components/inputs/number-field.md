...
숫자가 없고 포커스도 없는 동안 라벨은 필드 자신의 줄 위에 내려와 있다가 포커스나 첫 숫자에서 노치로 올라갑니다. 규칙 전체는 [MPTextField](./text-field#floatinglabel)에 있습니다.

`steppers="split"`은 라벨을 노치에 고정합니다. `startIcon`과 같은 이유로, 빼기 버튼이 정확히 내려온 라벨이 설 자리에 앉기 때문입니다.
