...
| `MPButton` 하나  | 3.0 kB     | 6.4 kB           |
| 컴포넌트 다섯 개 | 9.1 kB     | 9.4 kB           |
| 컴포넌트 열 개   | 13.4 kB    | 12.2 kB          |
| 내보내는 것 전부 | 129.7 kB   | 23.4 kB          |

마지막 줄은 한 가지를 일부러 빼 놓았습니다. `MPCodeBlock`의 문법은 49.6 kB 더 되고, 블록이 무언가를 칠할 때만 받아 오며, 코드 블록이 없는 페이지는 아예 받지 않습니다. `npm run measure`가 그것을 따로 찍는 이유입니다.
...
Base UI는 실제 다운로드의 더 큰 절반이고 두 열 어디에도 들어 있지 않습니다. 함께 번들하면 컴포넌트 다섯 개는 20.7 kB, 열 개는 94.9 kB가 됩니다. 다만 peer dependency이므로 그것을 쓰는 다른 것들과 공유되고, 버전은 여러분이 정합니다.
