...
Thirty units landed on `main`, each as its own commit with implementation, tests, bilingual documentation, a props row, a demo, a gallery entry and a changelog entry.
...
| Inputs | `MPTreeSelect` |
| Data | `MPDataTable`, `MPStatistic`, `MPSparkline`, `MPLineChart`, `MPAreaChart`, `MPBarChart`, `MPPieChart`, `MPScatterChart` |
| Feedback | `MPMeter`, `MPHoverCard`, `MPTour` |

Three units remain, listed below.
...
- **`MPHeatmapChart`** — two band scales and a sequential ramp rather than the categorical palette. A heatmap's cells all touch, so the colour job here is sequential (one hue, light to dark) and not the eight slots; `MPScatterChart`'s shapes are the other answer to the same cap and are no use to a cell.
...
Two things the `dataviz` skill settles that are not negotiable: **never a dual-axis chart** (two measures of different scale are two charts), and **a hover layer by default** on every one of the three. `MPSparkline` is the one exception and its documentation says why.
