...
- **ESM only**, TypeScript declarations included, and tree-shakeable for real: one component is 3.0 kB gzipped and five are 9.0 kB, against 107.5 kB for every export there is. `npm run measure` prints those numbers off a real bundler, so they cannot quietly stop being true. The stylesheet splits the same way.
...
A page that renders a handful of components can take the tokens and a sheet each instead of the whole 140 kB — 4.5 kB gzipped for one component against 20.4 kB for the lot. [The guide](https://material-plus.cdget.com/guide/getting-started#if-you-only-render-a-few-components) has the numbers and where the two paths cross.
