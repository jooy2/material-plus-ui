...
- **A chart axis turns a long label instead of cutting it.** Whichever axis runs along the bottom now reads `tickLabels`, and its default `auto` keeps the labels level while the widest of them fits its slot and turns them once it does not — 45° first, upright where even a turned label's line box will not fit between two ticks. `truncate` is the old behaviour, and `rotate` and `vertical` turn every label whether or not it needed it. `MPHeatmapChart`'s column names read the same option.
  ...
  It replaces the two things the axis used to do with a long name, both of which threw the name away: cutting it to "Onbo…", and showing every nth until the survivors fit. A turned label collides across its **line box** rather than across its own width, and a line box does not grow with the label — so twelve page names fit an axis that was showing four of them. Short labels are left alone however crowded the axis is, because an axis of "Jan", "Feb", "Mar" is crowded by how many of them there are and not by how long any one is — and so are ticks as opposed to names, so `auto` turns nothing along the bottom of a chart turned on its side or of a scatter, where that axis is a ruler and every nth loses nothing.

  The band a turned axis takes is capped at half the chart's height, and 120px whichever is smaller; past that the names are cut again, and a chart that genuinely needs more wants a taller `height` or a `thickness` of its own. Each label is measured against its own tick rather than against the first one, so only the leftmost is ever shortened to stay inside the chart.

- **`MPLineChart` and `MPAreaChart` take `gaps`, for the two readings a broken line gets wrong.** `connect` joins the two sides of a `null` and draws the joining run **dashed**, so the line carries on and the picture still says which part of it was measured; on an area the fill closes across the gap as well. `zero` draws the missing reading at zero and widens the value axis to hold it, which is right where a gap means "none of it happened" and wrong where it means "nobody was counting". `break`, the default, is what the two charts already did. Only the drawing changes in all three: the hover panel and the table behind the chart still report a gap as a gap.

- **`zoom` lets a reader drag a range out of a long category axis.** For the chart with ninety points on it, where the shape of one week is inside a picture of a quarter: a drag across the plot redraws it to what was picked, and the scale re-fits, so the window is a _chart of that range_ rather than a magnified picture of the whole one. The value scale, the hover panel, the spoken readout and the table behind the picture all follow it — the table most of all, since it is what a reader who cannot see the plot is given instead of it.

  The way back is a **Show all** control over the corner of the plot, named in all nineteen locales and a **sibling** of the picture rather than a child: `role="img"` is a leaf role, so a button inside it is one no keyboard can reach. `Escape` clears the reading first and the window second, which is the order the reader made them in, and a drag narrower than two categories is ignored — a window of one has no distance left in it for a line to have travelled, and a plain click on the plot is exactly that.

  Pass an object for `range` and `onRangeChange`, `defaultRange`, or a `min` wider than two. A range is clamped to the data rather than trusted, because one that outlives the series it was picked from is an empty chart with no way back to the full one. Only a chart whose categories are a row of slots takes it: `MPScatterChart` has two value axes and no row to cut, and crops with `xAxis` and `yAxis` instead.

- **The hover panel takes `sort`, and adds up a stacked column on its own.** `tooltip={{ sort: 'value' }}` puts the largest row first, which is what a chart of eight series wants: the row a reader is looking for is usually the big one. The default stays the order the series were passed, so a reader who has learned the legend can find a row without reading it, and ties keep their series order either way.

  A stack also gets a row for what the column adds up to, ruled off from the parts and carrying no swatch — the height of that column is the whole point of the shape, and a panel that listed the parts without it left the reader adding them up. An unstacked chart does not get one, because its series are not parts of anything; nor does a percent stack, whose answer is 100% every time. `tooltip={{ total }}` overrides both. The row is read out with the rest of the panel and is named in every locale the library ships.

- **`MPBarChart` and `MPAreaChart` take `stacked="percent"`.** Every bar is full and every band reaches the top, so the totals leave the picture and what is compared is the **composition** — the shape a share-of-traffic chart wants. It answers "what is this made of" and refuses to answer "how much of it is there", which is why it is a third setting rather than a flag on the second: a quarter of 1,200 and a quarter of 12 are drawn identically, so a total a reader needs belongs somewhere else on the page.

  The axis runs 0% to 100%, and the hover panel says both the value and its share, because the share is what the picture shows and the value is what the reader came for. A negative has no share of a whole and is left out of one, the way `MPPieChart` leaves it out; the table behind the chart still has the number. An area's value labels write each band's own share rather than the running total.

- **A chart axis takes `references`: the target line, the threshold and the band a reader brought with them.** `yAxis={{ references: [{ value: 300, label: 'SLO', color: 'error' }] }}` draws a dashed line across the plot at 300 with its name at the end of it; a reference with a `to` as well is a band, both edges drawn and the room between them filled. A `value` may be a `Date` on an axis of dates.

  It is deliberately not data: a reference wears the chrome's ink rather than a palette slot, it is dashed so it cannot be mistaken for a gridline, and nothing is inferred. **The scale opens to hold it**, because a target the plot is cropped above is the one thing a target must never be — pin `min` or `max` to stop that. A band is drawn under the marks and a line over them: a fill over a bar is a bar the reader has to look through, and a target behind the series that crossed it is a target nobody can read.

  A reference belongs to the axis whose scale its `value` is on and is drawn perpendicular to it, so the same one is a horizontal line upright and a vertical one on a chart turned on its side. Only an axis with a scale reads them — the value axis always, and the category axis where it runs on numbers, as `MPScatterChart`'s does. A named reference is read out beside the table, which has no column for it.

- **`MPSparkline` takes `gaps` too.** The same three readings the line and the area chart take: `connect` joins the two sides of a missing point with a dashed run and closes an area across it, and `zero` reads the point as a zero and opens the scale to hold it — which matters more on a mark with no axis, since nothing else would say the floor had moved. A `bar` has no line to join, so it takes `zero` and reads `connect` as `break`. The sentence the mark is read out as still counts only the readings it has.

- **Charts take `labelColor`, which writes the text naming a series in that series' colour.** It covers the legend's entries and the values written onto the marks. `ink` stays the default, and the reason is measurable rather than stylistic: the eight palette slots are fitted to 3:1 against the surface, which is the bar a _mark_ has to clear, and small text is held to 4.5:1. Labels drawn **on** a fill — a pie's shares, a heatmap's cells — keep the ink that reads against what they sit on, and `MPHeatmapChart` and `MPTimelineChart` do not take the prop at all.

- **`MPPieChart` takes `hole` and `gap`.** `hole` is how much of the radius a `donut` or a `semi` leaves empty, from `0` to `0.9` against the previous fixed `0.62`; `gap` is how much page shows between two slices, in pixels, `0` for slices that touch. The gap is still drawn as a stroke rather than by narrowing each sweep, so every slice keeps the angle its share earned however wide it is set.

- **`material-plus-ui/a2ui`: A2UI's basic catalog, drawn as Material Design 3.** [A2UI](https://a2ui.org) is a protocol for an agent to describe an interface instead of writing one: the agent sends JSON naming components from a _catalog_, and a renderer on the client draws them. The new subpath implements the protocol's own basic catalog — all eighteen of `Text`, `Image`, `Icon`, `Video`, `AudioPlayer`, `Row`, `Column`, `List`, `Card`, `Tabs`, `Modal`, `Divider`, `Button`, `TextField`, `CheckBox`, `ChoicePicker`, `Slider` and `DateTimeInput` — out of this library's components, so an agent's surface arrives as MD3 rather than as somebody's default styling.

  `mpA2uiCatalog` is registered under the basic catalog's own id, which makes it a drop-in: a `catalogId` is a promise about schemas and says nothing about how they are drawn, and these implementations are built from the `…Api` objects the protocol publishes rather than from copies of them. So an agent already targeting the basic catalog needs no change, and no schema has to be hosted anywhere for a client to announce it. `createMPA2uiCatalog` mints a catalog under an id of your own, with your own components beside these or in place of one of them.

  The two SDK packages it needs — `@a2ui/react` and `@a2ui/web_core` — are **optional peer dependencies**, because they bring Lit, signals, Zod, a date library and a Markdown parser with them and this package promises two runtime dependencies. Nothing in the library outside `src/a2ui` imports either one and the subpath is not in the barrel, so every bundle figure in this file still holds for a project that does not render agent interfaces. The catalog adds no CSS of its own either: it draws with components that are already styled, so `material-plus-ui/styles.css` and the split per-component sheets are unchanged, and the few declarations it does write are inline and read the `--mp-sys-*` tokens — an agent's surface follows `--mp-source-color` and `data-mp-shape` like everything else.

  Where the mapping had to choose, it chose MD3's answer: the protocol's `primary`, `default` and `borderless` buttons are the filled, outlined and text buttons, a filterable `ChoicePicker` is `MPCombobox` rather than a text box above a list, and `DateTimeInput`'s two switches pick between the date, time and date-time pickers — converting the payload's ISO 8601 strings to local dates by hand, so a day is never moved by a time zone on the way in or out. `Video` and `AudioPlayer` are the browser's own players with Material's corner on them, since neither is a component this library has. Markdown in a `Text` is rendered only when the application has configured a renderer through `@a2ui/react`'s Markdown context, whose contract requires it to sanitize; with none configured an agent's string is drawn as the characters it contains. The guide has a page on all of it.

- **`material-plus-ui/a2ui` also carries a catalog of this library's own: `DataTable`, `BarChart`, `LineChart`, `PieChart` and `Statistic`.** The basic catalog cannot describe a table or a chart, which is most of what an agent reporting numbers has to draw, so `mpA2uiExtendedCatalog` is the eighteen plus these five under `MP_A2UI_CATALOG_ID` — an id of ours, because five components the basic vocabulary cannot express are a different contract and must not claim to be that one. Register both and the agent chooses per surface.

  Their schemas are this library's rather than the protocol's, and are written for a caller that can only send JSON: a table's `rows` and a chart's `series` take a literal array or a path into the data model, a series states which accent role it reads rather than a colour of its own, and the props that are functions in a page — cell renderers, column accessors, `Intl` options — are replaced by the narrower questions an agent can answer. There is deliberately no row action: an A2UI action resolves its payload from data paths when it fires and cannot say which row was pressed, so selection carries that instead, through `selectedKeys` bound to a path.

  A value bound to a path is resolved rather than validated — the schema checked the binding, and the data model may hold something else by the time it is read — so what arrives is read rather than trusted. A number that is not a number is a gap, a series that is not a list is an empty chart, and a cell holding an object is written out as JSON instead of throwing where React refuses it.

- **The catalog's schema is published at the id it names**, [material-plus.cdget.com/a2ui/v0_9/catalog.json](https://material-plus.cdget.com/a2ui/v0_9/catalog.json), generated in `npm run build` from the same Zod schemas the renderer enforces — so the description an agent reads cannot drift from the contract a payload is checked against. The conversion is the SDK's, with its deduplication turned off: it points repeated subschemas at `#/properties/…` pointers that the component envelope leaves naming nothing, which is survivable on the wire and is a dangling reference in a file published at a URL. A test compares the committed file against the running catalog, and the build fails if the SDK's own output stops agreeing about which components exist and what they require.
...
- **A chart whose category names did not fit now turns them rather than cutting them.** That is `tickLabels` defaulting to `auto`, described above, and it is a visible change to any chart that was showing an ellipsis or dropping every other label — the plot loses height to the band the turned names take. A chart whose labels already fitted is drawn exactly as it was. `xAxis={{ tickLabels: 'truncate' }}` restores the old drawing.

- **A switched-off legend entry is drawn as a disabled control rather than struck through.** The swatch and the name fade together to Material's disabled opacity, which is the treatment a reader already knows for "this is off". A series merely standing back while another is hovered is now one step quieter than usual instead of sharing that fade, so the two states cannot be mistaken for each other. Neither depends on hue, so both survive greyscale and forced colours, and `aria-pressed` is unchanged.

- **Clicking a legend entry no longer leaves the plot faded behind the pointer.** The entry took focus on the click and focus dimmed every other series, so a chart sat in its hovered state until something else was clicked. Focus dims the rest only when it arrived from the keyboard now, which is what the dimming was for.
...
- **`MPImage` mirrors a picture with `flip`.** It takes `'horizontal'`, `'vertical'` or `'both'`, measured on the screen, so `horizontal` swaps left and right whether or not the picture is turned. The mirror is drawn with the `scale` property, which leaves `transform` free, and `preview` opens the picture mirrored.

- **`MPImage` takes `position`: which part of the picture a crop keeps.** It takes the centre, a side, a corner or two percentages, and also decides where `contain`, `none` and `scale-down` leave their empty space. The value is read on the picture as shown, so `top` keeps the top of a picture that is turned or mirrored, and it stays physical on a right-to-left page. A value in any other form reaches `object-position` unchanged.

- **`MPImage` fills the room `fit` leaves with `letterbox`.** `'blur'` draws the picture again behind itself, covering the box and blurred, the way a video player fills the sides of a portrait clip. The copy is hidden from assistive technology and the pointer, takes the picture's turn, mirror and position, and is given exactly what the picture loads from, so the file is fetched once. It is drawn only for `contain`, `none` and `scale-down`. Any other string is painted behind the picture as a CSS `background`.

- **`MPImage`'s `placeholder` takes a picture.** `{ src, blur }` draws a small copy of the file, a data URI or a `Blob` in place of the shimmer while the file arrives, fitted, placed, turned and mirrored like the picture. `blur: true` is a 20px radius and a number is a radius in pixels. The stand-in stays opaque until the picture has faded in over it and is then hidden in one step, so the page never shows through a cross-fade, and it is removed if the file fails. A `Blob` is drawn through an object URL that is revoked when the Blob is replaced or the image unmounts. Like the shimmer, it needs a reserved box.

- **`MPImage` takes `priority`, for the picture a page is judged by.** It sets `loading="eager"` and a high fetch priority, and a `loading` or `fetchPriority` written out on the image still wins. The attribute is spelled the way the running React knows it, `fetchPriority` in 19 and `fetchpriority` in 18, so neither warns. The documentation now describes `loading`, `decoding` and `fetchPriority`, which already passed through to the `<img>`.
...
- **`MPCarousel`'s arrows sit at 85% rather than solid.** A slide is usually a photograph, and two full-strength discs of the accent on one is chrome shouting over the thing it is there to move. The alpha is on the container and not on the button: `opacity` would take the glyph down with it, and the glyph against its own disc is the contrast that has to survive. Eighty-five is the lowest step that still clears 4.5:1 over a white slide against the default source — 4.73 there, against 5.04 at 88% and 4.26 at 80%. Hover, focus and a press take it back to solid, and a disabled arrow keeps the specification's own treatment. MD3 names no carousel arrow at all, so this is the library's decision rather than a token being followed.

- **The dependencies are current, and the test runner is on vitest 5.** `@base-ui/react` moves to 1.8, `lucide-react` to 1.47, React and its types to 19.3, and the rest of the toolchain to its newest. `eslint` and `typescript` are deliberately held where they were.

  `lucide-react` 1.47 is the one update a consumer can measure: it costs about 0.8 kB gzipped on any bundle that draws a glyph, which puts `MPButton` at 3.8 kB where it was 3.0 and the whole library at 130.6 where it was 129.7. A bundle with no glyph in it — `MPBox` — is unchanged. Nothing in the library's own code changed to meet any of these.

- **Every `Intl` formatter the library builds is now cached, keyed on what it was asked for.** Constructing one resolves a locale and loads the data behind the options; the object that comes back then formats for free, so a formatter built per value is the one cost worth not paying. The date module had worked this out and cached its own; the charts, `MPStatistic`, `MPSparkline`, `MPPieChart`, the time axis and the text segmenter behind `MPAnimateSplit` had not — a pie built a percent formatter per slice and a split built a segmenter per word. `MPAnimateCounter` was the worst of them: it memoised on an `options` **prop**, so a caller writing that object inline rebuilt the formatter on every frame of the count. The cache reads what the options say rather than which object said it, so an inline literal hits. `localeWeekStart` and `isHour12`, which the four pickers call in their render body, are cached on the locale as well.
...
- **The colour roles are drawn in browsers without relative colour syntax, which puts the floor at Chrome and Edge 111, Firefox 113 and Safari 16.4.** Every source-derived role is written with relative colour syntax, and a browser that cannot compute one drops it without a sound: in Chromium 111 and Firefox 113 all 24 were transparent, so a filled button had no fill and a dialog had no sheet behind its text, and WebKit 16.4 and 17.4 lost the tertiary family. `src/styles.css` now writes the whole default scheme out a second time, light and dark, as the colours a current browser draws, behind an `@supports not (…)` for the syntax the roles use. Chrome and Edge before 119, Firefox before 128 and Safari before 18 draw that copy; `--mp-source-color` changes nothing there, and `--mp-sys-color-*` still does.

  Measured in Playwright's builds of Chromium 111, Firefox 113, 127 and 128 and WebKit 16.4, 17.4 and 18.0, and in Chrome for Testing 122: all 58 role values are within two steps of a current Chromium's, and every documentation demo renders. `test/styles/relative-colour.test.tsx` holds the copy to the roles.
...
- **The README shows the components before it describes them.** A picture under the tagline puts the core controls around the mark — fields, a select, a switch, radios and checkboxes, buttons and chips, a segmented button, pagination, an alert, a slider, a progress bar, a meter, avatars, a badge and a rating. It is linked by absolute URL for the same reason the logo is: npm renders the README too, and the file is not in the tarball.
...
- **The size figures in the README and the guide are the current build's.** The whole stylesheet is 171.6 kB, 23.4 kB compressed, and the split sheets for one, five and ten components are 6.4, 9.4 and 12.2 kB; the JavaScript for five, ten and every component is 9.1, 13.4 and 129.1 kB. The old figures were measured for 1.7.0, before the fixes in this release, the colour changes above and below, and the additions to `MPImage`.
...
- **`MPStepper`'s panel has a name, and the step that opens it says so.** The panel is drawn outside the rail rather than inside the step it belongs to, so nothing in the markup said the two were related and the panel was a `role="group"` with an `aria-label` explicitly set to `undefined` — announced as "group" and nothing else. It takes its name from the title of the step that is open now, and a pressable step points at it with `aria-controls` and reports whether it is the one showing. A stepper whose steps carry no content promises no panel.

- **`MPTransfer`'s two lists are told apart.** Each list is a group named after the heading already above it. A reader moving by form control heard the row's label and nothing saying which side of the transfer it was on, on the one component whose whole subject is which side a row is on.
...
- **The ends of every palette are drawn in the colour they name.** A container or `on-` role takes its palette's full chroma at a tone where sRGB has only a fraction of it, or none, and the sheet left the browser to bring it in. Browsers do not do that at constant lightness and hue; Chromium, Firefox and WebKit all clip each channel on its own. The default `on-primary` was therefore `#b8ffff` rather than white — the label on every filled button — `primary-container` a saturated cyan, `on-error` a salmon and `error-container` a ΔE of 0.10 from MD3's. The accent and error roles now bring a colour that does not fit inside sRGB themselves: its chroma is reduced in OKLab at its own lightness and hue, by an estimate of what sRGB can hold there, and whatever still does not fit is moved towards the grey of that lightness in linear light. A colour that fits is left alone, and for the default source every role lands within a ΔE of 0.004 of an exact chroma reduction. Fed MD3's baseline source, the eighteen container and `on-` roles of the three accent families land within 0.02 of MD3's and the three `on-` roles of the light scheme on its white; a vivid source's containers stop drifting in hue. The colour page has a section on it. With the default scheme written out for older browsers as well, under Changed, the whole sheet is 2.1 kB larger compressed than it was.
...
- **A native control no longer wears the browser's own border, fill or type on a page without a reset.** The library ships no reset, so each component takes a `<button>`'s styling off itself — and most of them took off the background and the font family and left the border. A filled, tonal, elevated or text `MPButton` and `MPToggle`, a pressable `MPChip` and its ×, `MPFloatingActionButton`, `MPSegmentedButton`, `MPTabs`, `MPMenubar`, `MPNavigationMenu`, `MPBottomNavigation`, `MPStepper`'s steps, `MPNumberField`'s steppers, the triggers of `MPSelect`, `MPColorPicker`, `MPTreeSelect` and the four date and time pickers, `MPCombobox`'s input and buttons, `MPCalendar`'s days, the time picker's rows, `MPAlert`'s × and every chart's legend drew a two-pixel bevel, and several a grey fill or the browser's padding as well. `MPPill`, `MPDataTable`'s sort buttons, a chart's legend and `MPCombobox`'s input set their words in the browser's 13px, because `font-family: inherit` does not carry the size. The documentation site resets buttons on its own, which is why none of this showed there; a page with a reset of its own sees no change.

- **A list a component draws no longer wears the browser's margin, indent or markers on a page without a reset.** A `<ul>` or an `<ol>` arrives with a block margin of one em, forty pixels of indent and a marker on every item. `MPList`, `MPTimeline`, `MPNavigationMenu` and a chart's legend kept all three that applied — the legend drew a bullet in front of every series — and `MPBreadcrumb` and `MPStepper`'s rail kept the margin. A chart's tooltip had the same list inside it. Each now takes the three off itself, as `MPPagination`, `MPAnchor` and `MPFilePicker` already did.

- **A paragraph, quote or figure a component draws no longer wears the browser's margin on a page without a reset.** Base UI's `Field.Description` renders a `<p>`, which arrives with a block margin of one em, so the supporting text under `MPCheckbox`, `MPSwitch`, `MPSelect`, `MPCombobox`, `MPNumberField`, `MPOtpField`, `MPRadioGroup`, `MPColorPicker`, `MPTreeSelect` and the four date and time pickers sat an em below its control and an em above whatever came next, and `MPRadio`'s own description did the same around a 12px line. The margin also spoiled the line's entrance: the field jumped by both margins before the row began to open, and dropped by them again when an error replaced the description. `MPBlockquote` kept `1em 40px` on its `<blockquote>` and, with an attribution, on its `<figure>`, which set the words forty pixels in from their own rule.

  Each now says `m-0` on the element it owns — on the description only, since `Field.Error` renders a `<div>` with no margin. A host stylesheet that styles `blockquote` by name still outranks the zero, and `MPBlockquote`'s comment says why that is left alone. `test/styles/native-blocks.test.tsx` measures each against a bare `<p>`, `<blockquote>` and `<figure>` in the same page. The documentation site zeroes these margins inside its previews, which is why none of it showed there.

- **`MPChip` keeps the tails of its letters.** The label is truncated, and truncation clips at the line box, which the chip's one-em leading made shorter than the glyphs inside it — so every g, j, p, q and y lost its descender, at every size. The label's line box is now the font's own height. The chip is the same height and the words sit where they did.
...
- **`MPIcon` drew a glyph smaller than the `size` it was given when the glyph carried a margin.** The artwork is a flex item at `width: 100%` inside a box whose width is fixed, so `100% + margin` overflows and flex took the difference out of the glyph rather than letting it overflow: `<MPIcon icon={Tag} size={16} />` under a page rule of `[&_svg]:mr-1.25` rendered 11×16, with nothing to say so. A margin aimed at `svg` is what a migration off Material UI leaves behind, where the `SvgIcon` _was_ the element to put one on. The glyph slots are `shrink-0` now, so a declared size is what gets drawn and a stray margin overflows where it can be seen. The icon page says where spacing belongs.

- **The notch was cut wherever the page's `text-align` put it, and the label stayed at the inline start.** A rendered `<legend>` is placed within the border it interrupts by its `<fieldset>`'s `text-align`, and the outline declared none of its own, so it inherited whatever was around it. `MPFieldLabel` is absolutely positioned and inherits nothing. Put a field inside a centred container — a sign-in card is the obvious one — and the gap opened in the middle of the top border while the label sat on the left, 165px away from it on a 345px card. The outline now declares `text-align: start`, which is `start` rather than `left` so the notch keeps following the writing direction under RTL. Same six shells as above.
...
- **A dismissed `MPAlert` leaves instead of disappearing, and `onClose` fires when it has.** The press called back, the caller's answer to that is to stop rendering the alert, and so the element was gone in the same commit with nothing left to animate — and because an alert sits in the flow of the page rather than over it, everything under it jumped by the height of the message.

  Everything else in this library that leaves is portalled, and Base UI holds it open for the length of its own exit. An alert has nobody to do that for it, so it does it itself: the press starts the exit and `onClose` is the end of it, roughly 200ms later. **That is a change to when the callback fires**, and it is the moment the caller wanted — what an application does with `onClose` is unmount the alert, which is exactly what should happen once the exit has finished. The press is handed over with it, held rather than re-created, since React has not pooled events since 17.

  The space goes with the message: the row travels between `1fr` and `0fr`, which is the one way to interpolate towards a height nobody knows. An alert that only faded would leave a hole where it was and move the page anyway, 200ms later and no longer attached to the press that caused it.

  Three details that are not obvious and were each found the hard way. The alert is drawn _inside_ the collapsing row rather than being it, because a box with `box-sizing: border-box` cannot be shorter than its own padding and one asked to be the zero-height row bottomed out at 32px of empty container at `md`. `overflow: hidden` is on only while it is leaving, since an alert that went on clipping would cut the focus ring off its own dismiss button. And an alert with no transition to wait for — a reader who asked for reduced motion, or a page that took the transition off — reads the duration rather than waiting for a `transitionend` that is never coming.

- **A row that arrives in a list settles into it.** `MPTransfer` moves rows between its two panels and `MPFilePicker` adds them as files are chosen or dropped, and in both the row simply existed in the next frame. On a list of near-identical lines that is the change hardest to see: a reader who dropped four files onto a list that already held six had nothing telling them which four were new, and a transfer's whole gesture is "these ones went over there".

  A keyframe rather than a transition, for the reason the calendar's page turn is one — the row was not there, and a transition needs something that was. The part worth knowing is why the class is not simply on every row: a CSS animation runs when its element is inserted, so a blanket class looks like it would fire only for the rows the browser has just made, and it does not. Adding it to a row that is already drawn starts the animation there too, so the first move would replay every row in both panels. Both components keep the set of values that just landed and mark those.

  It says nothing about which way a row came, which `MPTransfer` could have. `translate` is physical, so the sign would have to be flipped by reading the computed direction under RTL, and it would be a slot `MPFilePicker` has no answer for. A rise settles a row into a list in either writing direction.

- **A field's supporting line opens rather than shoving the page down.** The line under a control is drawn only when there is something to say, so a field that turned invalid grew by a line of text between one frame and the next. On a form validated on blur that is the whole page stepping as the reader tabs through it.

  Two keyframes, because two different things happen. A field with nothing to say and one with something to say differ by a line of layout, and the row opens from `0fr`; a field whose description has been replaced by an error differs by the words and the space is already there, so that one is a fade. Opening a row that is already open would collapse the line and re-open it, which is a flinch rather than a change of message.

  A page load is not a change, and the animation is switched on by an attribute the component sets only once it has watched the message change — a class that was simply always on would grow six description lines out of nothing the moment a server-rendered form hydrated. A message _leaving_ is still instant: softening it would mean holding the last message the field had, and a field still showing an error it no longer has, to a screen reader as much as to the eye, is worse than a line that closes quickly.
